...
- **State Manager (`manager.py`):** Quản lý và duy trì trạng thái của hạ tầng AWS. Nó hoạt động như một lớp giao tiếp với **cơ sở dữ liệu PostgreSQL** để lưu trữ và truy xuất thông tin về các tài nguyên đã được phát hiện hoặc tạo ra.
...
Client --> FastAPI --> AI Agent --> LLM (lên kế hoạch) --> Plan Executor --> Tools --> AWS
  ^            |             ^                                      |
  |            |             | (trạng thái hạ tầng từ CSDL)          | (cập nhật qua WebSocket)
  |            +-------------+--------------------------------------+
  | (cập nhật)               |
  +--------------------------+
```
...
1.  **Quét hạ tầng:** Nó gọi `scanner.scan_aws_resources()` để tự động phát hiện trạng thái mới nhất của các tài nguyên trên AWS. Trạng thái này được cập nhật vào `StateManager` (và lưu xuống CSDL).
2.  **Thu thập ngữ cảnh:**
...
5.  **Cập nhật Context và State:**
    ...
    - Nếu một tài nguyên được tạo, `PlanExecutor` sẽ cập nhật `StateManager` (lưu vào CSDL) với thông tin về tài nguyên mới.
...
- **`agent_router.py`:**
    - Định nghĩa các endpoint RESTful:
        ...
        - `GET /state`: Lấy trạng thái hiện tại của hạ tầng từ CSDL.
...
- **`StateManager`:**
    - Đóng vai trò là lớp trừu tượng để quản lý trạng thái hạ tầng trong **cơ sở dữ liệu PostgreSQL**. Nó không còn lưu trữ trạng thái trong bộ nhớ.
    - **`set_discovered_state`:** Được gọi bởi agent sau khi quét AWS để cập nhật trạng thái trong CSDL.
    - **`add_resource`:** Được gọi bởi `PlanExecutor` sau khi một tài nguyên được tạo thành công để ghi thông tin tài nguyên mới vào CSDL.
    - **`get_current_state_formatted`:** Truy vấn CSDL để lấy dữ liệu trạng thái, sau đó định dạng lại thành một chuỗi văn bản dễ đọc để đưa vào prompt cho LLM.
...
- **`PlanExecutor`:**
    ...
    - **`_update_state_after_creation`:** Gọi `StateManager` để lưu lại thông tin về tài nguyên mới vào cơ sở dữ liệu.

## 4. Cấu hình (`.env` và `config.yaml`)

- **`.env`:** Chứa các biến môi trường nhạy cảm và đặc thù cho từng môi trường triển khai.
    - **`DATABASE_URL`:** Chuỗi kết nối đến cơ sở dữ liệu PostgreSQL. Đây là thay đổi quan trọng so với việc lưu trạng thái vào file.
    - `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `OPENAI_API_KEY`, v.v.
- **`config.yaml`:** Là file cấu hình trung tâm cho các cài đặt ít thay đổi hơn.
    - **`agent`:** Cho phép thay đổi LLM provider (`gemini`, `openai`), model, temperature,...
    - **`logging`:** Cấu hình mức độ log (debug, info, error).
    - Các giá trị trong `.env` sẽ ghi đè lên các giá trị tương ứng trong `config.yaml`, đảm bảo tính linh hoạt.
